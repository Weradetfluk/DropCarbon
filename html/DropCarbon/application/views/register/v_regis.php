	<br>
	<div class="container py-5">
    <h2 align="center" style="font-size:70px; text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.685);"><b>Drop Carbon System</b></h2>
    <br> 
	<div class="row">
	<div class="col">
	<a href=" <?php echo site_url().'/register/Tourist_regis/tourist_register_show'; ?>">
            <div class="h-100 py-5 services-icon-wap shadow">                         
                    <div class="h1 text-success text-center">
                        <i class="fa fa-user"></i>
                    </div>
            </div>
	</a>
            <div class="col">
                <h1 class="h2 mt-4 text-center">สำหรับนักท่องเที่ยว</h1>
            </div>
	</div>	
	<div class="col">
	<a href=" <?php echo site_url().'/register/Etp_input/show_regis_ent'; ?>">
            <div class="h-100 py-5 services-icon-wap shadow">                         
                    <div class="h1 text-success text-center">
                        <i class="fa fa-user"></i>
                    </div>              
            </div>
	</a>
            <div class="col">
                    <h1 class="h2 mt-4 text-center">สำหรับผู้ประกอบการ</h1>
            </div>     
	</div>
	</div>
	</div>

